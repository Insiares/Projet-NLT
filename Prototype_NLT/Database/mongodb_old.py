...
def update_database(prompt, result_, name, ret_output):
    client, collection = connect_mongodb()
    result = collection.find({"prompt": prompt, "result": result_,"username": name})
    result_ = ret_output
    # collection.update_one({"prompt": prompt, "result": result_,"username": name}, upsert=True)
    collection.update_many({"prompt": prompt, "result": result_, "username": name}, {"$set": {"result": result_}})
    client.close()
